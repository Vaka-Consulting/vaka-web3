...
export const useWalletExtendedStore = ({
  ...
  const localStorageKey = useMemo(
    () => `${projectPrefix}:wallet`,
    [projectPrefix]
  );

  const handleProjectAssets = useCallback(() => {
    const projectAssetsInWallet = assets.filter((asset) =>
      projectAssetsPolicyIds.includes(asset.policyId)
    );
    const projectToken = assets.find(
      (asset) => asset.policyId === projectTokenPolicyId
    );
    const projectTokenBalance =
      parseInt(projectToken?.quantity || "0") / LOVELACE_AMOUNT;
  ...
  const handleWalletConnectionIfStored = useCallback(
